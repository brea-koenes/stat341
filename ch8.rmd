...
```{r}
summarize_beta_binomial(alpha = 2, beta = 1, y = 1.5, n = 100)
```

```{r}
qbeta(c(0.25, 0.975), 3.5, 99.5)
```


There is a 95% probability that the true proportion of loons observed by a birdwatcher across a 100-hour observation period lies within 0.02-0.08%. 
...
I would say that the alternative hypothesis is too low. The credible interval indicates that the actual proportion is probable to be larger than 0.01, not smaller as the alternative hypothesis states. I would say that the alternative hypothesis is not very likely. 
