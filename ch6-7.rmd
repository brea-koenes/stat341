...
model <- "
  data {
    real y[4];
    y[1] = 7.1;  y[2] = 8.9;   y[3]  = 8.4; 
    y[4] = 8.6;
  }
  parameters {
    real<lower=4, upper=16> mu;
    real<lower=.1, upper=4> sigma;
  }
  model {
    y ~ normal(mu, sigma);
    mu ~ normal(10, 2);
...
#sim <- stan(model_code = model, iter = 5000*2, seed = 84735)
#plot(sim)
```
...
proposal_plaus <- dnorm(proposal, 0, 1) 
...
proposal_plaus <- dnorm(proposal, 0, 1) * bayestestR::distribution_uniform(-0.5,0.5)
